...
public class MineBlockAction extends BaseAction {
    ...
    public MineBlockAction(CraftoEntity crafto, Task task) {
        super(crafto, task);
    }
    ...
    protected void onStart() {
        ...
        if (nearestPlayer != null) {
            ...
            for (int y = lookTarget.getY(); y > lookTarget.getY() - 20 && y > -64; y--) {
                ...
                if (crafto.level().getBlockState(groundCheck).isSolid()) {
            ...
            crafto.teleportTo(miningStartPos.getX() + 0.5, miningStartPos.getY(), miningStartPos.getZ() + 0.5);
            
            String[] dirNames = {"North", "East", "South", "West"};
            int dirIndex = miningDirectionZ == -1 ? 0 : (miningDirectionX == 1 ? 1 : (miningDirectionZ == 1 ? 2 : 3));
            CraftoMod.LOGGER.info("Crafto '{}' mining {} in ONE direction: {}", 
                crafto.getCraftoName(), targetBlock.getName().getString(), dirNames[dirIndex]);
        } else {
            miningStartPos = crafto.blockPosition();
            currentTunnelPos = miningStartPos;
        ...
        CraftoMod.LOGGER.info("Crafto '{}' mining {} - staying at {} [SLOW & VISIBLE]", 
            crafto.getCraftoName(), targetBlock.getName().getString(), miningStartPos);
    ...
    protected void onTick() {
        ...
        if (ticksRunning > MAX_TICKS) {
            ...
            crafto.setItemInHand(InteractionHand.MAIN_HAND, net.minecraft.world.item.ItemStack.EMPTY);
        ...
        if (crafto.level().getBlockState(currentTarget).getBlock() == targetBlock) {
            crafto.teleportTo(currentTarget.getX() + 0.5, currentTarget.getY(), currentTarget.getZ() + 0.5);
            
            crafto.swing(InteractionHand.MAIN_HAND, true);
            
            crafto.level().destroyBlock(currentTarget, true);
            minedCount++;
            ticksSinceLastMine = 0; // Reset delay timer
            
            CraftoMod.LOGGER.info("Crafto '{}' moved to ore and mined {} at {} - Total: {}/{}", 
                crafto.getCraftoName(), targetBlock.getName().getString(), currentTarget, 
    ...
    private void placeTorchIfDark() {
        ...
        if (lightLevel < MIN_LIGHT_LEVEL) {
            ...
            if (torchPos != null && crafto.level().getBlockState(torchPos).isAir()) {
                ...
                CraftoMod.LOGGER.info("Crafto '{}' placed torch at {} (light level was {})", 
                    crafto.getCraftoName(), torchPos, lightLevel);
    ...
    private BlockPos findTorchPosition(BlockPos center) {
        ...
        if (crafto.level().getBlockState(floorPos).isSolid() && 
            crafto.level().getBlockState(center).isAir()) {
            return center;
        }
        ...
        for (BlockPos wallPos : wallPositions) {
            if (crafto.level().getBlockState(wallPos).isSolid() && 
                crafto.level().getBlockState(center).isAir()) {
                return center;
            }
        }
    ...
    private void mineNearbyBlock() {
        ...
        BlockState centerState = crafto.level().getBlockState(centerPos);
        if (!centerState.isAir() && centerState.getBlock() != Blocks.BEDROCK) {
            crafto.teleportTo(centerPos.getX() + 0.5, centerPos.getY(), centerPos.getZ() + 0.5);
            crafto.swing(InteractionHand.MAIN_HAND, true);
            crafto.level().destroyBlock(centerPos, true);
            CraftoMod.LOGGER.info("Crafto '{}' mining tunnel at {}", crafto.getCraftoName(), centerPos);
        }
        
        BlockState aboveState = crafto.level().getBlockState(abovePos);
        if (!aboveState.isAir() && aboveState.getBlock() != Blocks.BEDROCK) {
            crafto.swing(InteractionHand.MAIN_HAND, true);
            crafto.level().destroyBlock(abovePos, true);
        }
        
        BlockState belowState = crafto.level().getBlockState(belowPos);
        if (!belowState.isAir() && belowState.getBlock() != Blocks.BEDROCK) {
            crafto.swing(InteractionHand.MAIN_HAND, true);
            crafto.level().destroyBlock(belowPos, true);
        }
    ...
    private void findNextBlock() {
        ...
        for (int distance = 0; distance < 20; distance++) {
            ...
            for (int y = -1; y <= 1; y++) {
                ...
                if (crafto.level().getBlockState(orePos).getBlock() == targetBlock) {
        ...
        if (!foundBlocks.isEmpty()) {
            ...
            if (currentTarget != null) {
                CraftoMod.LOGGER.info("Crafto '{}' found {} ahead in tunnel at {}", 
                    crafto.getCraftoName(), targetBlock.getName().getString(), currentTarget);
            }
        }
    }
    ...
    private void equipIronPickaxe() {
        ...
        crafto.setItemInHand(net.minecraft.world.InteractionHand.MAIN_HAND, pickaxe);
        CraftoMod.LOGGER.info("Crafto '{}' equipped iron pickaxe for mining", crafto.getCraftoName());
    }
    ...
    private Block parseBlock(String blockName) {
        ...
        Map<String, String> blockMappings = new HashMap<>() {{
            // Ores
            put("iron", "iron_ore");
        ...
        if (blockMappings.containsKey(blockName)) {
            blockName = blockMappings.get(blockName);
        }
        ...
        if (block == Blocks.AIR && !blockName.equals("minecraft:air")) {
            CraftoMod.LOGGER.warn("Unknown block: {}, defaulting to stone", blockName);
            return Blocks.STONE;
        }
