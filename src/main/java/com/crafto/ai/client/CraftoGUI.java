...
public class CraftoGUI {
    ...
    public static void toggle() {
        ...
        if (isOpen) {
            initializeInputBox();
            mc.setScreen(new CraftoOverlayScreen());
            if (inputBox != null) {
            ...
            if (mc.screen instanceof CraftoOverlayScreen) {
                mc.setScreen(null);
            }
        }
    }
    ...
    private static void sendCommand(String command) {
        ...
        if (mc.player != null) {
            ...
            if (targetCraftos.size() > 1) {
                addSystemMessage("→ " + String.join(", ", targetCraftos) + ": " + command);
            } else {
                addSystemMessage("→ " + targetCraftos.get(0) + ": " + command);
            }
        }
    }
    
    private static List<String> parseTargetCraftos(String command) {
